...
class noticia extends Verificador
{
    ...
    public function guardarNoticia()
    {
        ...
        $params = array(
            $this->titulo, $this->descripcion,  $this->fechaFinal, 1,
    ...
    public function actualizarNoticia()
    {
        $sql = 'UPDATE noticia SET titulo_noticia = ?, descripcion_noticia = ?, fecha_final = ?,
           ...
           WHERE id_noticia = ?';
        $params = array(
            $this->titulo, $this->descripcion, $this->fechaFinal, $this->estadoNoticia,
    ...
    //Función para obtener datos para la gráfica
    public function obtenerDatos()
    {
        $sql = "SELECT p.nombre_producto, calcular_cantidad(p.id_producto) as total FROM noticia n
        INNER JOIN producto p ON p.id_producto = n.id_producto
        ...
        GROUP BY total, nombre_producto";
